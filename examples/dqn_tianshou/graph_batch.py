...
class TSGBatch(Batch):
    ...
    def graph_aggregate(self, batches):
        data_list = []
        for batch in batches:
            data_list.append(GData(**batch))
        gbatch = GBatch.from_data_list(data_list)
        for k in gbatch.keys:
            self.__dict__[k] = gbatch[k]
        #return Batch(**{k:gbatch[k] for k in gbatch.keys})
