...
class BilliardWorldMaze(PyGameWrapper):
    ...
    def init(self):
        ...
        self.reward_idx = self.rng.rand((self.N_CREEPS))
        self.reward_idx.sort()
        ...
        self._add_creep(0, 0, self.reward_idx[0])
        for i in range(self.N_CREEPS - 1):
            self._add_creep(1, i+1, self.reward_idx[i+1])
    ...
    def step(self, dt):
        ...
        if self.creep_counts["GOOD"] == 0 and self.creep_counts["BAD"] != 0:
            ...
            creep_new = Creep(
                (self.reward_idx[creep.idx]*200 + 40, self.reward_idx[creep.idx]*200 + 40, self.reward_idx[creep.idx]*200 + 40),
                self.CREEP_RADII[0],
